...
if __name__ == "__main__":
    args = get_args()
    # in_files = args.input
    in_path = '/home/xyj/data/spacenet/vegas/test/images/'
    out_path = 'output_cen/'
    if not os.path.exists(out_path):
        os.mkdir(out_path)

#     in_files = listdir(in_path)
    in_files = ['RGB-PanSharpen_AOI_2_Vegas_img81.tif',
                'RGB-PanSharpen_AOI_2_Vegas_img565.tif',
                'RGB-PanSharpen_AOI_2_Vegas_img1295.tif',
                'RGB-PanSharpen_AOI_2_Vegas_img1546.tif'
               ]
    ...
    for i, fn in enumerate(in_files):       # 问题：不用mini_batch吗？不用，训练时用
        ...
        img = Image.open(in_path+fn).convert('RGB')
